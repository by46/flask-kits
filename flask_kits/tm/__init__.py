...
current_tm = LocalProxy(lambda: app.extensions['kits_tm_sender'])


def init_extension(kits, app):
    sender = TMSender(logger=app.logger)
    app.extensions['kits_tm_sender'] = sender


class TMSender(object):
    ...
    @property
    def template_id_url(self):
        access_token = JsApi.token
    ...
    def send_message(self, message_id, open_id, redirect_url, **kwargs):
        try:
            ...
            response = self.session.post(url, json=pay_load, verify=False)
            result = response.json()
            import json
            self.logger.error('debug %s', json.dumps(pay_load))
            if result['errcode']:
                self.logger.error(u'send template message failure, error code: %s, error message: %s',
